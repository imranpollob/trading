def trade_summary_with_fractional(coin_price, investment_amount, current_coin_price=None, fee_percent=0.25):
  # Input validation
  if coin_price <= 0 or investment_amount <= 0 or fee_percent < 0:
      raise ValueError("Invalid input: coin_price, investment_amount must be > 0 and fee_percent >= 0")
  ...
  if current_coin_price:
      sell_fee_at_current = current_coin_price * quantity * (fee_percent / 100)  # Sell fee at current price
      total_sell_value_at_current = current_coin_price * quantity - sell_fee_at_current  # Net sell value
      profit_at_current = total_sell_value_at_current - actual_total_cost  # Profit or loss at current price
      percentage_profit_loss = (profit_at_current / actual_total_cost) * 100  # Percentage profit/loss
  else:
...
# Input data
coin_price = 116  # Price per coin or unit
investment_amount = 100  # Amount invested in dollars
current_coin_price = 120  # Current market price of the coin (optional)

# Run the function
trade_summary_with_fractional(coin_price, investment_amount, current_coin_price)
...
# trade_summary(buy_price, quantity)
